...
char device_id[] = 		"428ef978e8d64a7e88c0b3e0c9d43b52";
char device_token[] = 	"24698374f525464a8b230c132de24b2f";
    ...
    while(1) {
    	...
    	if (nbytes < 0) {
    	    ...
    	    if (nsamples * sizeof(struct adc_msg_s) != nbytes) {
    	    	...
    	    	if((con_in==false)&&(con_out==false))
    	    	{
    	    		if((sample.am_channel==0)&&(sample.am_data<500))
    	    		{
    	    			con_in=true;
    	    		}
    	    	}else if((con_in==true)&&(con_out==false))
    	    	{
    	    		if((sample.am_channel==0)&&(sample.am_data<500))
    	    		{
    	    			con_in=false;
    	    			...
    	    			gpio_write(LED, 1);
    	    			char buf[40];
    	    			sprintf(buf, "{\"inhome\" : %d}", inthehouse);
    	    			mqtt_msg_t message;
    	    		...
    	    		}else if((sample.am_channel==1)&&(sample.am_data<500)){
    	    			con_out=true;
    	    			inthehouse=false;
    	    			gpio_write(LED,0);
    	    			char buf[40];
    	    	...
    	    	}else if((con_in==true)&&(con_out==true))
    	    	{
    	    		if((sample.am_channel==1)&&(sample.am_data<500)){
    	    			con_out=false;
    	    		}
    	    	}
