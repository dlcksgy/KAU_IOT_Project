...
char device_id[] = 		"f79066c4811c4913bcc15999aa315291";//개인의 디바이스의 관함
char device_token[] = 	"7e3c8815fd4a47aeb6c1f6e65f6f8c32";
...
#endif
{
    ...
    int fd;
    struct adc_msg_s sample;//sensor input value
    size_t readsize;
    ssize_t nbytes;
    int publishedVal = -1;
    ...
    while(1) {
    	...
    	if (nbytes < 0) {
    	    ...
    	    if (nsamples * sizeof(struct adc_msg_s) != nbytes) {
    	        ...
    	        if ((sample.am_channel == 0) && ((sample.am_data > -1 || (sample.am_data < -1)))) // A0 ADC
    	        {
    	        	if(sample.am_data <= 600){ sample.am_data=true;}
    	        	else sample.am_data=0;
    	            printf("Publish message for signal A0\n");
    	            char buf[40];
    	            sprintf(buf, "{\"fire\" : %d}", sample.am_data);
    	            printf("fire Value: %d\n", sample.am_data);
    	            //char* msg = buf;
